			...
			if ($result->num_rows > 0) {
			    while ($row = $result->fetch_assoc()) { ?>
			        ...
			        <div style="background-color: rgba(0, 0, 0, 0.7); width: 50%; border: 3px solid white; border-radius: 15px;padding: 10px; margin-bottom: 20px; display: flex;align-items: center;box-shadow: 0 0 8px #db0000; border-style: none; font-size: 24px;">
			        	...
			        	<div style="width: 70%;">
			        		<h2 style="text-align:center;">Profile <?php echo $row["userID"] ?></h2>
							<div style="display: flex; justify-content: space-between;margin-right: 20px; margin-left: 20px;">
							    <p>Name:&ensp;<?php echo $row["Name"]; ?></p>
							    <p>Email:&ensp;<?php echo $row["Email"]; ?></p>
							</div>
				            <div style="display: flex; justify-content: space-between;margin-top: -10px;margin-right: 20px; margin-left: 20px;">
							    <p>DateOfBirth:&ensp;<?php echo $row["DateOfBirth"]; ?></p>
							    <p>Password:&ensp;<?php echo $row["Password"]; ?></p>
							</div>
				            <?php echo "<button>
							   				<a href='EditProfile.php?id=" . $row['userID'] . "' style='text-decoration: none; color: black;'>
							            		<b>Edit</b>
							        		</a>
							     		</button>";
							?>
				            <?php echo "<button style='margin-left: 50px'>
        									<a href='Profile_delete.php?id=" . $row['userID'] . "'onclick=\"return confirm('Are you sure you want to delete this profile?');\" style='text-decoration: none; color: black;'>
								           		<b>Delete</b>
								       		</a>
								        </button><br><br>";
							?>
			        ...
			        <hr style="height: 2px; background-color: #db0000; border: none;margin-left: 80px;margin-right: 80px;">
				<?php }
				} else {
